...
\begin{itemize}
  ...
  \item When this fails, OLS is \alert{inconsistent}: $\widehat{\beta}_1^{OLS} \xrightarrow{p} \beta_1 + \dfrac{\operatorname{cov}(X_i, u_i)}{\operatorname{var}(X_i)} \neq \beta_1$.
...
\[
\widehat{\beta}_1^{TSLS} \xrightarrow{p} \beta_1 + \frac{\operatorname{cov}(Z_i, u_i)}{\operatorname{cov}(Z_i, X_i)}
\]
...
\[
\widehat{\beta}_1^{OLS} \xrightarrow{p} \beta_1 + \underbrace{\beta_2 \cdot \frac{\operatorname{cov}(X_i, W_i)}{\operatorname{var}(X_i)}}_{\text{Omitted Variable Bias}}
\]
...
\begin{align*}
\widehat{\beta}_1^{OLS} &\xrightarrow{p} \beta_1 + \frac{\operatorname{cov}(X_i, u_i)}{\operatorname{var}(X_i)} = \beta_1 + \frac{-\beta_1 \sigma_w^2}{\sigma_{X^*}^2 + \sigma_w^2} \\[6pt]
...
\begin{frame}{Simultaneous Causality -- deriving the bias (1/3)}
\label{slide:simul_bias}
\textbf{Goal}: Estimate the demand elasticity $\alpha_1$ from $Q_i = \alpha_0 + \alpha_1 P_i + u_i^d$.

\pause
\vspace{0.5em}
Recall the OLS probability limit formula:
\[
\widehat{\alpha}_1^{OLS} \xrightarrow{p} \alpha_1 + \frac{\operatorname{cov}(P_i,\, u_i^d)}{\operatorname{var}(P_i)}
\]

\pause
We need two ingredients from the equilibrium price $P_i = \underbrace{\frac{\gamma_0 - \alpha_0}{\alpha_1 - \gamma_1}}_{\bar{P}\text{ (constant)}} + \frac{u_i^s - u_i^d}{\alpha_1 - \gamma_1}$:

\begin{enumerate}
  \item $\operatorname{cov}(P_i,\, u_i^d)$
  \item $\operatorname{var}(P_i)$
\end{enumerate}

\pause
\vspace{0.3em}
\textbf{Assumptions}: $u^d \perp u^s$, both mean-zero with variances $\sigma_d^2$ and $\sigma_s^2$.
\end{frame}

\begin{frame}{Simultaneous Causality -- deriving the bias (2/3)}
\textbf{Step 1}: Compute $\operatorname{cov}(P_i,\, u_i^d)$.

Since $\bar{P}$ is a constant, only the stochastic part matters:
\[
\operatorname{cov}(P_i,\, u_i^d) = \operatorname{cov}\!\left(\frac{u_i^s - u_i^d}{\alpha_1 - \gamma_1},\; u_i^d\right) = \frac{1}{\alpha_1 - \gamma_1}\Big[\underbrace{\operatorname{cov}(u_i^s, u_i^d)}_{=\,0} - \operatorname{var}(u_i^d)\Big] = \frac{-\sigma_d^2}{\alpha_1 - \gamma_1}
\]

\pause
Since $\alpha_1 < 0 < \gamma_1$, we have $\alpha_1 - \gamma_1 < 0$, so $\operatorname{cov}(P_i, u_i^d) > 0$. \alert{Positive demand shocks raise the equilibrium price.}

\pause
\vspace{0.5em}
\textbf{Step 2}: Compute $\operatorname{var}(P_i)$.
\[
\operatorname{var}(P_i) = \operatorname{var}\!\left(\frac{u_i^s - u_i^d}{\alpha_1 - \gamma_1}\right) = \frac{1}{(\alpha_1 - \gamma_1)^2}\Big[\sigma_s^2 + \sigma_d^2\Big]
\]
(using $u^d \perp u^s$ so the cross-covariance vanishes)
\end{frame}

\begin{frame}{Simultaneous Causality -- deriving the bias (3/3)}
\textbf{Step 3}: Substitute into the probability limit.

\[
\widehat{\alpha}_1^{OLS} \xrightarrow{p}\; \alpha_1 + \frac{\operatorname{cov}(P_i, u_i^d)}{\operatorname{var}(P_i)} = \alpha_1 + \frac{\dfrac{-\sigma_d^2}{\alpha_1 - \gamma_1}}{\dfrac{\sigma_s^2 + \sigma_d^2}{(\alpha_1 - \gamma_1)^2}}
\]

\pause
Simplify:
\[
= \alpha_1 + \frac{-\sigma_d^2}{\alpha_1 - \gamma_1} \cdot \frac{(\alpha_1 - \gamma_1)^2}{\sigma_s^2 + \sigma_d^2} = \alpha_1 - \frac{\sigma_d^2(\alpha_1 - \gamma_1)}{\sigma_s^2 + \sigma_d^2}
\]

\pause
Combine into a single fraction:
\[
= \frac{\alpha_1(\sigma_s^2 + \sigma_d^2) - \sigma_d^2(\alpha_1 - \gamma_1)}{\sigma_s^2 + \sigma_d^2} = \frac{\alpha_1 \sigma_s^2 + \cancel{\alpha_1 \sigma_d^2} - \cancel{\alpha_1\sigma_d^2} + \gamma_1\sigma_d^2}{\sigma_s^2 + \sigma_d^2}
\]
...
\begin{enumerate}
  ...
  \item \textbf{Second stage}: Regress $Y$ on the predicted $\widehat{X}$ from stage 1.
\end{enumerate}

\pause
\vspace{0.5em}
\begin{exampleblock}{Intuition}
The first stage \emph{purifies} $X$ by projecting it onto the instrument $Z$. The predicted value $\widehat{X}$ inherits $Z$'s exogeneity.  The second stage uses only this purified variation.
\end{exampleblock}
\end{frame}
...
$\widehat{X}_i$ is the \textbf{fitted value} from the first stage.
\end{frame}

\begin{frame}{Second stage: using the purified $\widehat{X}$}
\label{slide:second_stage}
\textbf{Second stage}: Regress $Y_i$ on $\widehat{X}_i$ using OLS.

\[
Y_i = \beta_0 + \beta_1 \widehat{X}_i + \text{error}_i
\]

The OLS estimators from this second-stage regression are the TSLS estimators $\widehat{\beta}_0^{TSLS}$ and $\widehat{\beta}_1^{TSLS}$.
...
\begin{frame}{Deriving the TSLS formula (1/3)}
\label{slide:tsls_derivation}
\textbf{Starting point}: TSLS is OLS of $Y$ on $\widehat{X}$, so by the OLS slope formula:
\[
\widehat{\beta}_1^{TSLS} = \frac{s_{\widehat{X}Y}}{s_{\widehat{X}}^2}
\]
where $s_{\widehat{X}Y}$ is the sample covariance of $\widehat{X}$ and $Y$, and $s_{\widehat{X}}^2$ is the sample variance of $\widehat{X}$.

\pause
\vspace{0.5em}
\textbf{Key substitution}: The first stage gives $\widehat{X}_i = \widehat{\pi}_0 + \widehat{\pi}_1 Z_i$. Its sample mean is:
\[
\bar{\widehat{X}} = \widehat{\pi}_0 + \widehat{\pi}_1 \bar{Z}
\]

\pause
Therefore the deviation from the mean simplifies to:
\[
\boxed{\widehat{X}_i - \bar{\widehat{X}} = \widehat{\pi}_1(Z_i - \bar{Z})}
\]

\vspace{0.3em}
\textbf{Intuition}: All variation in $\widehat{X}$ comes from $Z$ through the first-stage slope $\widehat{\pi}_1$. The intercept $\widehat{\pi}_0$ cancels out.
\end{frame}

\begin{frame}{Deriving the TSLS formula (2/3)}
Substitute $\widehat{X}_i - \bar{\widehat{X}} = \widehat{\pi}_1(Z_i - \bar{Z})$ into each piece.

\pause
\vspace{0.5em}
\textbf{Numerator} -- sample covariance of $\widehat{X}$ and $Y$:
\[
s_{\widehat{X}Y} = \frac{1}{n-1}\sum_{i=1}^n \widehat{\pi}_1(Z_i - \bar{Z})(Y_i - \bar{Y}) = \widehat{\pi}_1 \underbrace{\frac{1}{n-1}\sum_{i=1}^n (Z_i - \bar{Z})(Y_i - \bar{Y})}_{=\,s_{ZY}} = \widehat{\pi}_1 \, s_{ZY}
\]

\pause
\vspace{0.3em}
\textbf{Denominator} -- sample variance of $\widehat{X}$:
\[
s_{\widehat{X}}^2 = \frac{1}{n-1}\sum_{i=1}^n \widehat{\pi}_1^2(Z_i - \bar{Z})^2 = \widehat{\pi}_1^2 \underbrace{\frac{1}{n-1}\sum_{i=1}^n (Z_i - \bar{Z})^2}_{=\,s_Z^2} = \widehat{\pi}_1^2 \, s_Z^2
\]
...
\textbf{Ratio}:
\[
\widehat{\beta}_1^{TSLS} = \frac{s_{\widehat{X}Y}}{s_{\widehat{X}}^2} = \frac{\widehat{\pi}_1 \, s_{ZY}}{\widehat{\pi}_1^2 \, s_Z^2} = \frac{s_{ZY}}{\widehat{\pi}_1 \, s_Z^2}
\]
\end{frame}

\begin{frame}{Deriving the TSLS formula (3/3)}
We have $\widehat{\beta}_1^{TSLS} = \dfrac{s_{ZY}}{\widehat{\pi}_1 \, s_Z^2}$. One last substitution.

\pause
\vspace{0.5em}
\textbf{Recall}: The first-stage OLS slope is:
\[
\widehat{\pi}_1 = \frac{s_{ZX}}{s_Z^2}
\]

\pause
Substitute:
\[
\widehat{\beta}_1^{TSLS} = \frac{s_{ZY}}{\left(\dfrac{s_{ZX}}{s_Z^2}\right) \cdot s_Z^2} = \frac{s_{ZY}}{\dfrac{s_{ZX} \cdot \cancel{s_Z^2}}{\cancel{s_Z^2}}} = \boxed{\frac{s_{ZY}}{s_{ZX}}} \qed
\]

\pause
\vspace{0.3em}
\begin{block}{Interpretation}
The TSLS estimator is the ratio of:
\begin{itemize}
  \item \textbf{Reduced form} $s_{ZY}$: how much $Y$ moves when $Z$ moves (total effect of $Z$ on $Y$ through $X$)
  \item \textbf{First stage} $s_{ZX}$: how much $X$ moves when $Z$ moves
\end{itemize}
This is the \alert{Wald estimator}: $\widehat{\beta}_1^{TSLS} = \dfrac{\text{reduced form}}{\text{first stage}}$.
\end{block}
\end{frame}
...
\[
\widehat{\beta}_1^{TSLS} = \frac{s_{ZY}}{s_{ZX}} \xrightarrow{p} \frac{\operatorname{cov}(Z_i, Y_i)}{\operatorname{cov}(Z_i, X_i)} = \beta_1 \tag{12.7}
\]
...
\[
\boxed{\widehat{\beta}_1^{TSLS} = \beta_1 + \frac{\frac{1}{n}\sum_{i=1}^n (Z_i - \bar{Z})u_i}{\frac{1}{n}\sum_{i=1}^n (Z_i - \bar{Z})(X_i - \bar{X})}} \tag{12.20}
\]
...
\[
\widehat{\beta}_1^{TSLS} \stackrel{a}{\sim} N\!\left(\beta_1, \;\sigma_{\widehat{\beta}_1^{TSLS}}^2\right)
\]
where
\[
\sigma_{\widehat{\beta}_1^{TSLS}}^2 = \frac{1}{n}\cdot\frac{\operatorname{var}\!\big[(Z_i - \mu_Z)u_i\big]}{\big[\operatorname{cov}(Z_i, X_i)\big]^2} \tag{12.8}
\]
\end{block}
...
Because $\widehat{\beta}_1^{TSLS}$ is asymptotically normal:

\begin{itemize}
  \item \textbf{$t$-statistic}: $t = \dfrac{\widehat{\beta}_1^{TSLS} - \beta_{1,0}}{SE(\widehat{\beta}_1^{TSLS})}$

  \medskip
  \item \textbf{95\% confidence interval}: $\widehat{\beta}_1^{TSLS} \pm 1.96 \cdot SE(\widehat{\beta}_1^{TSLS})$
\end{itemize}
...
\]
Obtain predicted values $\widehat{X}_{1i}, \ldots, \widehat{X}_{ki}$.

\vspace{0.5em}
\textbf{Second stage}: Regress $Y_i$ on $\widehat{X}_{1i}, \ldots, \widehat{X}_{ki}, W_{1i}, \ldots, W_{ri}$ using OLS.

\vspace{0.5em}
The resulting estimators are $\widehat{\beta}_0^{TSLS}, \widehat{\beta}_1^{TSLS}, \ldots, \widehat{\beta}_{k+r}^{TSLS}$.
\end{block}
...
Let $\widehat{X}_{ji}^*$ be the predicted value from the \emph{population} regression of $X_{ji}$ on all $Z$'s and $W$'s. Then $(\widehat{X}_{1i}^*, \ldots, \widehat{X}_{ki}^*, W_{1i}, \ldots, W_{ri}, 1)$ are \textbf{not perfectly multicollinear}.
...
\pause
Under these assumptions, $\widehat{\beta}^{TSLS}$ is \alert{consistent} and \alert{asymptotically normal}.
\end{frame}
...
From Eq.\ (12.20): $\widehat{\beta}_1^{TSLS} = \beta_1 + \bar{q}/\bar{r}$ where:
\begin{align*}
...
\[
\widehat{\beta}_1^{TSLS} - \beta_1 \approx \frac{\sigma_q}{\sigma_r}\cdot\frac{\bar{q}/\sigma_{\bar{q}}}{\bar{r}/\sigma_{\bar{r}}} = \frac{\sigma_q}{\sigma_r}\cdot\frac{N(0,1)}{N(0,1)}
\]
...
\[
E(\widehat{\beta}_1^{TSLS}) - \beta_1 \approx \frac{\beta_1^{OLS} - \beta_1}{E(F) - 1}
\]
\begin{itemize}
...
\begin{enumerate}
  ...
  \[
  \widehat{u}_i^{TSLS} = Y_i - \widehat{\beta}_0^{TSLS} - \widehat{\beta}_1^{TSLS}X_{1i} - \cdots - \widehat{\beta}_{k+r}^{TSLS}W_{ri}
  \]

  \item Regress $\widehat{u}_i^{TSLS}$ on all instruments $Z_{1i},\ldots,Z_{mi}$ and all exogenous regressors $W_{1i},\ldots,W_{ri}$. Compute the homoskedasticity-only $F$-statistic testing $\delta_1 = \cdots = \delta_m = 0$.
...
\begin{itemize}
  \item Compute TSLS using instrument 1 alone: $\widehat{\beta}_{1}^{(1)}$
  \item Compute TSLS using instrument 2 alone: $\widehat{\beta}_{1}^{(2)}$
\end{itemize}
...
\[
\widehat{\beta}_1^{(1)} \xrightarrow{p} \beta_1, \quad \widehat{\beta}_1^{(2)} \xrightarrow{p} \beta_1
\]
...
\[
E(\widehat{\beta}_1^{TSLS}) - \beta_1 \approx \frac{\overbrace{\beta_1^{OLS} - \beta_1}^{\text{OLS bias}}}{E(F) - 1}
\]
...
\begin{enumerate}
  ...
  \item TSLS works in two stages: (1) regress $X$ on $Z$'s and $W$'s; (2) regress $Y$ on $\widehat{X}$'s and $W$'s.
